...
function vowelCount(str){
    // Remove consonants
    let vowelsArr = str.split("").filter(function(char) {
        const vowels = "aeiou"
        return vowels.indexOf(char.toLowerCase()) !== -1
    })

    // Convert to lower case
    vowelsArr = vowelsArr.map(function(vowel) {
...
function findInObj(arr, key, searchValue) {
    return arr.filter(function(element, i) {
            return element[key] === searchValue && i === arr.indexOf(element)
        })[0]
}
